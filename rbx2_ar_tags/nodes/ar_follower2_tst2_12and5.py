...
class ARFollower():
	...
	def set_cmd_vel(self,msg):
		...
		if (self.marker_id==5 or self.marker_id==12):
			...
			if (target_offset_x - self.goal_x) > self.x_threshold:
				speed =  abs((self.goal_x - (target_offset_x))) * self.x_scale #target_offset_x +0.2
				if speed < 0:
					speed = 0.1#1.5
		...
		elif((self.TargetFlag is not True) and ((self.marker_id != 5 ) or (self.marker_id != 12))):

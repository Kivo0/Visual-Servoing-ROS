...
class ARFollower():
	...
	def set_cmd_vel(self,msg):
		...
		if (self.marker_id==5):
			...
			if ((target_offset_x - self.goal_x) > self.x_threshold and self.sm_markerFlag == False and self.stopSearchingTargetLostFlag == False and self.stopSearchingTargetLostFlag == False):
			...
			elif(self.stopSearchingTargetLostFlag == False):
				...
				if((target_offset_x <= 0.95 ) and (target_offset_x >= 0.4) and (cspeed <= 0.1) and (self.stopSearchingTargetLostFlag == False)):#0.8 0.05 0.1, respectively
				...
				elif(self.sm_markerFlag==True and self.stopSearchingTargetLostFlag == False):
		...
		if (self.rightFlag==True and self.AlignedZeroFlag==True and self.parkingfinished_R == False):
			rospy.loginfo("WE CAME FROM THE RIGHT\n")
			
			self.stopSearchingTargetLostFlag = True
			if ((self.stopspinning==False) and (target_offset_x <= 2) and (self.stopSearchingTargetLostFlag==True)):
			...
			if (self.marker_id !=5):#((self.target_visible == False) and (self.marker_id !=5)):
				...
				if((self.move_cmd.angular.z>=-0.01) and (self.move_cmd.angular.z<=0.01) and (self.targetlostonpurposeflag == True) and (self.stopmoving==False)):		
					...
					time.sleep(2)
					self.stopmoving=True
					rospy.loginfo("1st IF\n")
				if(self.stopmoving==True):
					self.move_cmd.linear.x = 0
					rospy.loginfo("last IF\n")


		if((self.stopmoving==True) and (self.move_cmd.linear.x>=-0.02) and (self.move_cmd.linear.x<=0.02) and (self.parkingfinished_R == False)):
			self.move_cmd.angular.z = -0.15 # will be opposite in left
			rospy.loginfo("I GOT to return  right\n")
			if(yaw>-2.0 and yaw<-1.71): #from 0.8 to 1.4
				#right blind alignment
				self.move_cmd.angular.z = -0.25
				rospy.loginfo("parking right aligning -- Z turning right\n")	
				...
				rospy.loginfo("Parking is ABSOLUTELY DONE right:3\n")

			elif(yaw>-1.46 and yaw < -1.25):# from -0.8 to -1.4 yaw<-0.8 and yaw > -1.7 yaw>-1.9 and yaw < -1.51
				#left alignment
				self.move_cmd.angular.z = 0.25
				rospy.loginfo("parking right aligning ++ Z turning left \n")
		...
		if((self.stopmoving==True) and (self.move_cmd.linear.x>=-0.02) and (self.move_cmd.linear.x<=0.02) and (self.parkingfinished == False)):
			...
			rospy.loginfo("I GOT to return \n")
			if(yaw>-2.0 and yaw<-1.71): #from 0.8 to 1.4
				#right blind alignment
				self.move_cmd.angular.z = -0.25
				rospy.loginfo("parking left aligning -- Z turning right\n")	
				...
				rospy.loginfo("Parking left is ABSOLUTELY DONE left :3\n")

			elif(yaw>-1.46 and yaw < -1.25):# from -0.8 to -1.4 yaw<-0.8 and yaw > -1.7 yaw>-1.9 and yaw < -1.51
				#left alignment
				self.move_cmd.angular.z = 0.25
				rospy.loginfo("parking left aligning ++ Z turning left \n")

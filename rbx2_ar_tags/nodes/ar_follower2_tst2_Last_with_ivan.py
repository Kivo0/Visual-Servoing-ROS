...
class ARFollower():
	...
	def __init__(self):
		...
		self.y_scale = rospy.get_param("~y_scale", 0.95)
		...
		#Left
		self.stopmoving = rospy.get_param("~stopmoving", False)
		...
		self.targetlostonpurposeflag = rospy.get_param("~targetlostonpurposeflag", False)
		self.parkingfinished = rospy.get_param("~parkingfinished", False)
		#Right
		self.stopmoving_R = rospy.get_param("~stopmoving_R", False)
		self.sm_markerFlag_R= rospy.get_param("~sm_markerFlag_R", False)
		self.AlignedZeroFlag_R = rospy.get_param("~AlignedZeroFlag_R", False)
		self.stopSearchingTargetLostFlag_R = rospy.get_param("~stopSearchingTargetLostFlag_R", False)
		self.stopspinning_R = rospy.get_param("~stopspinning_R", False)
		self.targetlostonpurposeflag_R = rospy.get_param("~targetlostonpurposeflag_R", False)
		self.parkingfinished_R = rospy.get_param("~parkingfinished_R", False)
		...
		self.targetlostonpurposeflag=False
		self.stopspinning=False
		self.parkingfinished=False
		#Right
		self.AlignedZeroFlag_R = False
		self.stopSearchingTargetLostFlag_R=False
		self.targetlostonpurposeflag_R=False
		self.stopspinning_R=False
		self.parkingfinished_R=False

		self.stopmoving_R = False
		self.sm_markerFlag_R = False
	...
	def set_cmd_vel(self,msg):
		...
		try:
			...
			if ((self.TargetFlag is not True) and (self.marker_id !=5) and (self.stopSearchingTargetLostFlag==False) and (self.stopSearchingTargetLostFlag_R==False)):
		...
		if (self.marker_id==5):
			...
			if ((target_offset_x - self.goal_x) > self.x_threshold and self.sm_markerFlag == False and self.stopSearchingTargetLostFlag == False and self.stopSearchingTargetLostFlag_R == False):
			...
			elif(self.stopSearchingTargetLostFlag == False and self.stopSearchingTargetLostFlag_R == False):
				...
				if((target_offset_x <= 1 ) and (target_offset_x >= 0.37) and (cspeed <= 0.1) and (self.stopSearchingTargetLostFlag == False) and (self.stopSearchingTargetLostFlag_R == False)):#0.8 0.05 0.1, respectively
					self.move_cmd.linear.x = cspeed*e**(0.44)-cspeed*e**(cspeed)+0.085 # 0.4 worked cspeed*0.1+0.16

					rospy.loginfo("increasing Speed \n")
					self.sm_markerFlag=True
				elif(self.sm_markerFlag==True and self.stopSearchingTargetLostFlag == False and self.stopSearchingTargetLostFlag_R == False):
				...
				# self.move_cmd.linear.z = 0.0
					...
					if(yaw>-2.0 and yaw<-1.71 and self.AlignedZeroFlag==False): #from 0.8 to 1.4
						#right blind alignment
						self.move_cmd.angular.z = -0.25
						rospy.loginfo("target aligning -- Z turning right\n")
					       	self.rightFlag=True

					elif(yaw>-1.7 and yaw <=-1.45 and self.AlignedZeroFlag==False):# from -1.7 to -1.4
						#center
						self.move_cmd.angular.z = 0.0
					...
					elif(yaw>-1.46 and yaw < -1.25  and self.AlignedZeroFlag==False):# from -0.8 to -1.4 yaw<-0.8 and yaw > -1.7 yaw>-1.9 and yaw < -1.51
						#left alignment
						self.move_cmd.angular.z = 0.25
		...
		if (self.rightFlag==True and self.AlignedZeroFlag==True and self.parkingfinished == False and self.parkingfinished_R == False):
			rospy.loginfo("WE CAME FROM THE RIGHT\n")
			self.stopSearchingTargetLostFlag_R=True
			if ((self.stopspinning_R==False) and (target_offset_x<=1.9) and (self.stopSearchingTargetLostFlag_R==True)):
				self.move_cmd.angular.z = 0.30
			if (self.marker_id !=5):#((self.target_visible == False) and (self.marker_id !=5)):
				self.stopspinning_R=True
				self.move_cmd.angular.z /= 1.065 #1.07 1.02
				rospy.loginfo("I LOST THIS TARGET ON PURPOSE from right :3\n")
				rospy.loginfo(" the current z speed is : %f",self.move_cmd.angular.z)
				#time.sleep(2)
				self.targetlostonpurposeflag_R = True				
				if((self.move_cmd.angular.z>=-0.01) and (self.move_cmd.angular.z<=0.01) and (self.targetlostonpurposeflag_R == True) and (self.stopmoving_R==False)):		
					self.move_cmd.linear.x = 0.02
					time.sleep(2)
					self.stopmoving_R=True
					rospy.loginfo("1st IF RIGHT\n")
				if(self.stopmoving_R==True):
					self.move_cmd.linear.x = 0
					rospy.loginfo("last IF RIGHT\n")



		elif(self.leftFlag==True and self.AlignedZeroFlag == True and self.parkingfinished == False):
			rospy.loginfo("WE CAME FROM THE LEFT\n")
			self.stopSearchingTargetLostFlag=True
			if ((self.stopspinning==False) and (target_offset_x<=1.9) and (self.stopSearchingTargetLostFlag==True)):
				self.move_cmd.angular.z = -0.30
			if (self.marker_id !=5):#((self.target_visible == False) and (self.marker_id !=5)):
				...
				rospy.loginfo(" the current z speed is : %f",self.move_cmd.angular.z)
				#time.sleep(2)
				self.targetlostonpurposeflag = True				
				if((self.move_cmd.angular.z>=-0.01) and (self.move_cmd.angular.z<=0.01) and (self.targetlostonpurposeflag == True) and (self.stopmoving==False)):		
					self.move_cmd.linear.x = 0.02
					time.sleep(2)
					self.stopmoving=True
					rospy.loginfo("1st IF LEFT\n")
				if(self.stopmoving==True):
					self.move_cmd.linear.x = 0
					rospy.loginfo("last IF LEFT\n")



		if((self.stopmoving==True) and (self.move_cmd.linear.x>=-0.02) and (self.move_cmd.linear.x<=0.02) and (self.parkingfinished == False)):
			self.move_cmd.angular.z = 0.15
			rospy.loginfo("I GOT to return from the left! >_< \n")
			if(yaw>-2.0 and yaw<-1.71): #from 0.8 to 1.4
				#right blind alignment
				self.move_cmd.angular.z = -0.25
				rospy.loginfo("parking aligning -- Z turning right\n")	

			elif(yaw>-1.7 and yaw <=-1.45):# from -1.7 to -1.4
				#center
				self.move_cmd.angular.z = 0.0
				#rospy.loginfo("parking aligned\n")
				csOFZ=self.move_cmd.angular.z
				self.parkingfinished = True
				rospy.loginfo("Parking is ABSOLUTELY DONE from the left :3\n")

			elif(yaw>-1.46 and yaw < -1.25):# from -0.8 to -1.4 yaw<-0.8 and yaw > -1.7 yaw>-1.9 and yaw < -1.51
				#left alignment
				self.move_cmd.angular.z = 0.25
				rospy.loginfo("parking aligning ++ Z turning left \n")
		...
		elif((self.stopmoving_R==True) and (self.move_cmd.linear.x>=-0.02) and (self.move_cmd.linear.x<=0.02) and (self.parkingfinished_R == False)):
			self.move_cmd.angular.z = -0.15 # will be opposite in right
			rospy.loginfo("I GOT to return from the right! >_< \n")
			if(yaw>-2.0 and yaw<-1.71): #from 0.8 to 1.4
				#right blind alignment
				self.move_cmd.angular.z = -0.25
				rospy.loginfo("parking aligning -- Z turning right\n")	
				...
				#rospy.loginfo("parking aligned\n")
				csOFZ=self.move_cmd.angular.z
				self.parkingfinished_R = True
				rospy.loginfo("Parking is ABSOLUTELY DONE from the right :3\n")

...
class ARFollower():
    ...
    def __init__(self):
        ...
        self.x_threshold = rospy.get_param("~x_threshold", 0.5) #0.01
        ...
        self.counter = rospy.get_param("~counter",0)
    ...
    def set_cmd_vel(self, msg):
		...
		#self.counter+=1
		#rospy.loginfo("the frame count is : %f", self.counter)
        ...
        if (target_offset_x - self.goal_x) > self.x_threshold:
            speed =  abs((self.goal_x - (target_offset_x))) * self.x_scale
            if speed > 0:
                speed *= 1.5
            self.move_cmd.linear.x = copysign(min(self.max_linear_speed, max(self.min_linear_speed, abs(speed))), speed)
        else:
            self.move_cmd.linear.x /= 3.9 #1.9
            cspeed =  self.move_cmd.linear.x
            rospy.loginfo("the current speed is : %f",cspeed)
            #if((target_offset_x <= 0.8 ) and (target_offset_x >= 0.40) and (cspeed <= 0.1)):#0.8 0.05 0.1
                #self.move_cmd.linear.x = 0.05  # 0.4 worked previous - 0.3
                #rospy.loginfo("increasing Speed \n")
	    #else:
                #self.move_cmd.linear.x /= 2.0 # 1.01
                #self.move_cmd.linear.y = 0.0 # 0.0
                #self.move_cmd.linear.z = 0.0 # 0.0
                
                #rospy.loginfo("target achieved\n")
	    if((target_offset_x <= 1.5 )): #0.8
		rospy.loginfo("SLOWING!\n")
                self.move_cmd.linear.x /= 4.0 # 1.01
                self.move_cmd.linear.y = 0.0 # 0.0
                self.move_cmd.linear.z = 0.0 # 0.0
